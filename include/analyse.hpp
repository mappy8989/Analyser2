...
auto AnalyseFunctions(const std::vector<std::string> &files,
    ...
    rs::for_each(files, [&](const auto &filename) {
        auto obj_funcs = analyser::function::FunctionExtractor().Get(filename);
        rs::for_each(obj_funcs,
                     [&](const auto &elem) { metrics.emplace_back(std::make_pair(elem, metric_extractor.Get(elem))); });
    });
    return metrics;
}
...
auto SplitByFiles(
    ...
    rs::sort(sorted_funcs, [](const auto &a, const auto &b) { return a.first.filename < b.first.filename; });
