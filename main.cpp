...
int main(int argc, char *argv[]) {
    ...
    const std::vector<std::string> filenames(std::move(options.GetFiles()));
    ...
    metric_acc.RegisterAccumulator(lines_counter.Name(), std::make_unique<MAI::AverageAccumulator>());
    metric_acc.RegisterAccumulator(param_counter.Name(), std::make_unique<MAI::CategoricalAccumulator>());
    metric_acc.RegisterAccumulator(cyclomatic_counter.Name(), std::make_unique<MAI::SumAverageAccumulator>());

    auto print_acc_results = [&](auto &&chunk) {
        analyser::AccumulateFunctionAnalysis(chunk, metric_acc);
        auto acc_res = metric_acc.GetFinalizedAccumulator<MAI::AverageAccumulator>(lines_counter.Name());
        std::println("{}: {}", lines_counter.Name(), acc_res.Get());
        auto sum_acc_res = metric_acc.GetFinalizedAccumulator<MAI::SumAverageAccumulator>(cyclomatic_counter.Name());
        std::println("{}: {}", cyclomatic_counter.Name(), sum_acc_res.Get().average);
        auto cat_acc_res = metric_acc.GetFinalizedAccumulator<MAI::CategoricalAccumulator>(param_counter.Name());
        std::ranges::for_each(cat_acc_res.Get(), [&](const auto &elem) {
            std::println("{}: {} {}", param_counter.Name(), elem.first, elem.second);
        });
        std::println();
    };
