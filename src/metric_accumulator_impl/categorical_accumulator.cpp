...
void CategoricalAccumulator::Accumulate(const metric::MetricResult &metric_result) {
    auto [metric_element, is_inserted] =
        categories_freq.insert({metric_result.metric_name, metric_result.value});
    ...
    if (is_inserted) {
        ...
        metric_element->second =
            metric_element->second * (n / (n + 1)) + (metric_result.value / (n + 1));
